...
class XzapSocksProxy(
    ...
    companion object {
        ...
        private const val MAX_POOL_WARMING_PARALLEL = 3 // max simultaneous TLS handshakes during background pool warming only
    ...
    // Throttle simultaneous TLS handshakes during background pool warming only.
    // NOT used on the hot path (openXzapTunnel) — blocking there adds latency
    // directly visible to the user (images stall after ~pool_size connections).
    private val poolWarmSemaphore = java.util.concurrent.Semaphore(MAX_POOL_WARMING_PARALLEL)
    ...
    private fun createPoolConnection() {
        // Throttle background pool warming — don't flood the server with TLS
        // handshakes during warmup. Not applied on the hot path (openXzapTunnel).
        if (!poolWarmSemaphore.tryAcquire()) return
